...
// 1. The Attractive "Wake Up" Screen
const ServerWakeupPage = ({ onServerReady, serverUrl }) => {
  useEffect(() => {
    const checkServer = async () => {
      try {
        const res = await fetch(`${serverUrl}/ping`);
        if (res.ok) {
          onServerReady(); // Transition to chat
        } else {
          throw new Error("Not ready");
        }
      } catch (e) {
        // If failed, retry in 3 seconds
        setTimeout(checkServer, 3000);
      }
    };
    checkServer();
  }, [serverUrl, onServerReady]);

  return (
    <div className="min-h-screen w-full bg-slate-950 flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans selection:bg-cyan-500/30">
      {/* Background Ambience */}
      <div className="absolute inset-0 z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] bg-indigo-600/10 rounded-full blur-[120px] animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] bg-cyan-600/10 rounded-full blur-[120px] animate-pulse delay-1000" />
        <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-10 brightness-100 contrast-150"></div>
      </div>

      {/* Main Card */}
      <div className="relative z-10 flex flex-col items-center max-w-md w-full text-center space-y-8 animate-in fade-in duration-1000 slide-in-from-bottom-10">
        {/* Logo / Icon */}
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-tr from-indigo-500 to-cyan-400 blur-xl opacity-40 animate-pulse"></div>
          <div className="relative bg-slate-900/80 border border-white/10 p-6 rounded-2xl shadow-2xl backdrop-blur-xl">
        ...
        <div className="space-y-3">
          <h1 className="text-3xl font-bold text-white tracking-tight">
            Initializing NovaMind
          </h1>
          <p className="text-slate-400 text-sm max-w-xs mx-auto leading-relaxed">
            Establishling secure connection to the Neural Core. This may take a
            moment for cold starts.
          </p>
        </div>

        {/* The Glowing Scanner Line Loader */}
        <div className="w-full max-w-[280px] h-1.5 bg-slate-800 rounded-full overflow-hidden relative">
          {/* The moving gradient line */}
          <div className="absolute top-0 left-0 h-full w-1/2 bg-gradient-to-r from-transparent via-cyan-400 to-transparent animate-[shimmer_1.5s_infinite_linear]"></div>

          {/* CSS Animation injection for 'shimmer' since we aren't in a CSS file */}
          <style>{`
            @keyframes shimmer {
        ...
        {/* Status Badge */}
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/5 text-xs font-medium text-slate-400 animate-pulse">
          <Wifi size={12} />
...
const App = () => {
  // -- STATE --
  const [isServerReady, setIsServerReady] = useState(false); // Controls which screen is shown
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (
        ...
        errorMessage = "Lost connection to NovaMind Core.";
      } else if (err.message) {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "bot", text: `⚠️ SYSTEM ERROR: ${errorMessage}` },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  // 1. Show Wake Up Screen if Server isn't ready
  if (!isServerReady) {
    return (
      <ServerWakeupPage
        serverUrl={ServerURI}
  ...
  // 2. Show Chat Interface once Server is ready
  return (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <span className="flex w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399]"></span>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
          {messages.map((msg, index) => (
            ...
            >
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-lg mt-1 border border-white/5 ${
                ...
                <div className="flex flex-col gap-1 min-w-0">
                  {msg.fileName && (
                    <div
                      className={`text-[10px] flex items-center gap-1 mb-1 px-2 py-0.5 rounded-md w-fit opacity-80 ${
                        msg.role === "user"
                  ...
                  <div
                    className={`p-3.5 md:p-4 rounded-2xl shadow-sm text-[15px] leading-relaxed whitespace-pre-wrap break-words ${
                      msg.role === "user"
        ...
        <div className="shrink-0 p-4 bg-slate-900/80 backdrop-blur-xl border-t border-white/10 z-20">
          {attachedFile && (
            <div className="mx-auto max-w-4xl mb-3 flex items-center gap-3 bg-slate-800/80 w-fit px-3 py-2 rounded-xl border border-white/10 shadow-lg animate-in slide-in-from-bottom-2">
              ...
              <span className="text-xs text-slate-200 truncate max-w-[200px] font-medium">
                {attachedFile.name}
              </span>
              <button
                onClick={removeFile}
                className="text-slate-400 hover:text-red-400 hover:bg-white/5 p-1 rounded-full transition-colors"
              >
                <X size={14} />
              </button>
            </div>
          ...
          <form
            onSubmit={handleSend}
            className="relative flex items-end gap-2 max-w-4xl mx-auto bg-black/20 border border-white/10 rounded-2xl p-1.5 focus-within:ring-2 focus-within:ring-indigo-500/50 focus-within:border-transparent transition-all shadow-inner"
          >
            <input
              type="file"
              ref={fileInputRef}
